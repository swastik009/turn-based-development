...
— specifically its [`/build`](https://github.com/addyosmani/agent-skills/blob/main/.claude/commands/build.md)
command and the `incremental-implementation` skill behind it. If you already use that, read this
before adding another skill on top, because the overlap is real: both work one unit at a time,
both run a test-driven loop per unit, both commit each unit separately.

The difference is **which side of the commit the human stands on.**

`/build`'s default loop ends: *"7. Commit with a descriptive message → 8. Mark the task complete
and stop."* The commit lands, and then it stops. The gate is that tests passed.

Here, the turn ends *before* the commit, and the agent asks two things — is this message right,
and please review this — every time, with approval never carrying to the next commit. The gate is
that a human read the diff.

Both are defensible. They optimise for different things:

| | `/build` | this |
|---|---|---|
| Unit of work | a task from the plan, may span files | one coherent block, one file |
| Human gate | after the commit; upfront-only in `auto` mode | before every commit, always |
...
**Use `/build`** when you trust the plan and want throughput — you have a spec, the tasks are
well-understood, and passing tests are enough assurance.

**Use this** when the point is that you personally understand and can maintain the result, and
you are willing to be present for it. It is slower and it does not have an autonomous mode,
because an autonomous mode would defeat the reason it exists.

Rule 11 is a debt to `/build`, which stops on *"anything you can't undo with `git revert`"* — a
